...
//ajax请求路径配置 
    ...
    employee.requestUrl = {//RentMarket2.0项目名  usback.do这个是所请求的servlet 
        queryList:employee.url+"rentMarket3.1/usback.do"  //数据是从servlet一侧返回的 json格式
    };
...
//批量删除的按钮实现方法 
	//选取第一列的CheckBox时获取该行的Id，如uuids[i].userId
	//获取所有选中行的UUID
	function batchIds() {
		...
		if (uuids.length == 0) {
			...
			for (var i = 0; i < uuids.length; i++) {
				...
				uuid.push(uuids[i].userId);//把选中的行的Id放到数组里 
			}
			// alert(uuid);
			layer.confirm('确认要删除吗？', function(index) {//ajax请求 
				$.get("../usback.do", "ids=" + uuid + "&op=MuchDel",
